...
class NoteViewSet(ModelViewSet):
   ...
   #http_method_names = ['get','post']
    ...
    def perform_create(self,serializer):
        serializer.save(author = self.request.user) # подвязываем автора под note


""" 3class NoteListView(ListCreateAPIView):
    queryset = Note.objects.all()
    serializer_class = NoteSerializer
...
class NoteDetailView(RetrieveUpdateDestroyAPIView):
    ...
    serializer_class = NoteSerializer
 """
""" 2class NoteListView(ListModelMixin, CreateModelMixin, GenericAPIView):
    queryset = Note.objects.all()
    serializer_class = NoteSerializer
...
""" 1class NoteListView(APIView):
    def get(self, request, format=None):
        notes = Note.objects.all()
        context = {'request':request} #надо надо
        serializer = ThinNoteSerializer(notes, many=True, context=context)#и тут его передать не забудь
        return Response(serializer.data)
